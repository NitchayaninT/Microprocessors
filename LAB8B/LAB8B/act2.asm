...
.EQU	KEY_PIN = PIND
.EQU	KEY_DDR = DDRD
; Set PORTB (D4-D7) as output for data lines
LDI  R20, 0xF0  ; Set D4-D7 as output (0b11110000)
OUT  DDRB, R20   ; Write to DDRB (Data Direction Register for PORTB)

; Set PORTC (PC0, PC1, PC2) as output for control lines (RS, RW, E)
LDI  R20, 0x07  ; Set PC0, PC1, PC2 as output (0b00000111)
OUT  DDRC, R20   ; Write to DDRC (Data Direction Register for PORTC)

 	; Initialize LCD (4-bit mode)
LDI  R20, 0x38  ; Function set: 4-bit mode, 2 lines, 5x8 font
OUT  PORTB, R20  ; Send to data pins D4-D7
LDI  R20, 0x01  ; Clear display command
OUT  PORTB, R20  ; Send clear command

; More initialization commands can go here
